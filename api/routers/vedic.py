...
from api.core.dashas import get_mahadashas, get_antardashas, get_current_dasha, get_yogini_dashas, get_ashtottari_dashas, get_char_dashas
...
async def vedic_navamsa(request: NatalChartRequest):
    ...
    return build_divisional_chart(positions, 9, request.options.ayanamsa.value, jd)
...
async def vedic_divisional_charts(request: NatalChartRequest):
    jd = birth_data_to_jd(request.birth_data)
    positions = get_all_planet_positions(jd, zodiac_type="SIDEREAL", ayanamsa=request.options.ayanamsa.value)
    return build_all_divisional_charts(positions, jd, request.options.ayanamsa.value)
...
@router.post("/dashas/char")
async def vedic_char_dasha(request: NatalChartRequest):
    jd = birth_data_to_jd(request.birth_data)
    bd = request.birth_data
    houses = get_houses(jd, bd.latitude, bd.longitude, "WHOLE_SIGN", "SIDEREAL", request.options.ayanamsa.value)
    return get_char_dashas(houses)
